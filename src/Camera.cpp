  ...
  pWindow(pWin)
{
    ...
    // initiale Projektion (wird in update() aus fov erneut gesetzt)
    m_ProjMatrix.perspective((float)M_PI * fov / 180.0f,
                             (float)WindowWidth/(float)WindowHeight,
                             0.045f, 1000.0f);
}

void Camera::setFOV(float fovNew)
{
    // sanftes Angleichen, damit FOV‑Sprünge vermieden werden
    const float factor = 0.05f;
    fov += (fovNew - fov) * factor;
}
...
void Camera::setTargetPosition(const Vector& Target, float factor)
{
    // Behalte den relativen Offset zwischen Pos und Target bei und lerpe dahin
    Vector offset = m_Position - m_Target;
    m_Target = Target;
    m_Position = m_Position * (1.0f - factor) + (m_Target + offset) * factor;
}
...
{
    Vector p( 1.0f*x/(float)WindowWidth*2.0f - 1.0f,
              1.0f*y/(float)WindowHeight*2.0f -1.0f,
              0);
    p.Y = -p.Y;
...
void Camera::update()
{
    // Aktuelles FOV in die Projektionsmatrix übernehmen
    m_ProjMatrix.perspective((float)M_PI * fov / 180.0f,
                             (float)WindowWidth / (float)WindowHeight,
                             0.045f, 1000.0f);
